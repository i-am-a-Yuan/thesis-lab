# -*- coding: utf-8 -*-
"""
Created on Fri Mar 27 02:22:26 2026
...
for name, w in weights.items():
    ...
    # SDEM（Spatial Durbin Error Model，本质是 SEM + WX）
    sdem = ML_Lag(
        y,
        X.values,
        w=w,
    ...
    betas = np.array(sdem.betas).flatten()

    std = np.array(sdem.std_err).flatten()

    pvals = [i[1] for i in sdem.z_stat]
    ...
    # ------------------
    if hasattr(sdem,"pr2"):
        table.loc["R2",name] = f"{sdem.pr2:.3f}"

    # ------------------
    # LogLik / AIC / SC
    # ------------------
    table.loc["Log Likelihood",name] = f"{sdem.logll:.3f}"
    table.loc["Akaike Info Criterion",name] = f"{sdem.aic:.3f}"
    table.loc["Schwarz Criterion",name] = f"{sdem.schwarz:.3f}"
...
# 输出
# ---------------------------
table.to_excel("SDEM_result_multiW_2k.xlsx")
