# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 21:02:21 2026
...
axes[0][0].text(0.95, 0.95, 
             ...
             fontsize=9)
# %% 2. 道路交叉口数量箱线图  road2 ln
axes[0][1].boxplot(np.log(df['road2']), patch_artist=True,
                boxprops=dict(facecolor=colors[1], color='black'),
                medianprops=dict(color='yellow', linewidth=2),
                whiskerprops=dict(color='black'),
                capprops=dict(color='black'),
                flierprops=dict(marker='o', markerfacecolor='red', markersize=5, alpha=0.5))
axes[0][1].set_title('对数化街镇道路交叉口数量', fontsize=14, fontweight='bold')
...
# 添加统计信息
pop_stats = np.log(df['road2']).describe()
...
# %% 3. gdp箱线图  gdp ln
axes[0][2].boxplot(np.log(df['gdp']), patch_artist=True,
                boxprops=dict(facecolor=colors[2], color='black'),
                medianprops=dict(color='yellow', linewidth=2),
                whiskerprops=dict(color='black'),
                capprops=dict(color='black'),
                flierprops=dict(marker='o', markerfacecolor='red', markersize=5, alpha=0.5))
axes[0][2].set_title('对数化街镇GDP总量', fontsize=14, fontweight='bold')
...
# 添加统计信息
pop_stats = np.log(df['gdp']).describe()
...
# %% 4. price箱线图  price
axes[1][0].boxplot(np.log(df['price']), patch_artist=True,
                boxprops=dict(facecolor=colors[3], color='black'),
                medianprops=dict(color='yellow', linewidth=2),
                whiskerprops=dict(color='black'),
                capprops=dict(color='black'),
                flierprops=dict(marker='o', markerfacecolor='red', markersize=5, alpha=0.5))
axes[1][0].set_title('对数化街镇单位面积平均房价', fontsize=14, fontweight='bold')
...
# 添加统计信息
pop_stats = np.log(df['price']).describe()
...
# %% 5. poi箱线图  poi
axes[1][1].boxplot(np.log(df['poi']), patch_artist=True,
                boxprops=dict(facecolor=colors[4], color='black'),
                medianprops=dict(color='yellow', linewidth=2),
                whiskerprops=dict(color='black'),
                capprops=dict(color='black'),
                flierprops=dict(marker='o', markerfacecolor='red', markersize=5, alpha=0.5))
axes[1][1].set_title('对数化街镇POI数量', fontsize=14, fontweight='bold')
...
# 添加统计信息
pop_stats = np.log(df['poi']).describe()
